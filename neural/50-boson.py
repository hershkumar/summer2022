...
def sample(
    ...
    keep,
    stepsize,
    positions_initial=INITIAL_SAMPLE,
    progress=False,
):
    sq = []
    sq_prime = []
...
def sample_body(params, coords_t, rand_coords, rand_accepts):
    ...
    return jax.lax.cond(
        rand_accepts < psi(coords_prime, params) ** 2 / psi(coords_t, params) ** 2,
        lambda x, _: (x, True),
        lambda _, y: (y, False),
        coords_prime,
        coords_t,
...
def Hpsi(coords, coords_prime, params, alpha):
    return Hpsi_without_delta(coords, params) + delta_potential(
        coords, coords_prime, params, alpha
    )
...
def Hpsi_without_delta(coords, params):
    ...
    return (
        1
        / (2 * m)
        * (jnp.sum(d2A_dx2(coords, params)) - jnp.sum(dA_dx(coords, params) ** 2))
        + m * 0.5 * harmonic_omega**2 * jnp.sum(coords**2)
        + sigma_term
    )
...
def third_term(coords, coords_prime, params, y_max):
    return dnn_dtheta(coords_prime, params) * delta_potential(
        coords, coords_prime, params, y_max
    )
...
def delta_potential(coords, coords_prime, params, alpha):
    ...
    delta_dist = (1 / (jnp.sqrt(jnp.pi) * alpha)) * jnp.exp(
        -(coords[1] ** 2) / (alpha**2)
    )
...
def gradient(
    params, num_samples=10**3, thermal=200, skip=50, variation_size=1.0, verbose=False
):
    # get the samples
    samples, samples_prime, _ = sample(
        params, num_samples, thermal, skip, variation_size
    )
    ...
    third_term = (
        1 / num_samples * jnp.sum(vthird_term(samples, samples_prime, params, alpha), 0)
    )
...
start_params = nn.flatten_params()
# start_params = jnp.load(PARAMS_FILE, allow_pickle=True)


opt_init, opt_update, get_params = jax_opt.adam(10 ** (-3))

resultsa = train(start_params, 30, 1500, 100, 10, find_step_size(start_params, 0.16))
# 0 -> energies
# 1 -> uncert
# 2 -> steps
# 3 -> params
...
resultsb = train(resultsa[3], 50, 10000, 500, 10, find_step_size(resultsa[3], 0.16))

# %%
resultsc = train(resultsb[3], 15, 100000, 500, 10, find_step_size(resultsb[3], 0.16))

# %%
resultsd = train(resultsc[3], 10, 800000, 500, 10, find_step_size(resultsc[3], 0.16))
...
def harmonic_energy(n):
    return 0.5 + 2 * n - np.sqrt(2 / np.pi) * (n + 1 / 2) / g
