...
num_particles = 2
N = num_particles
structure = [50,100,200,300,200,100,50]
...
resultsa = train(start_params, 100, 500, 100, 10, find_step_size(start_params, 1))

opt_init, opt_update, get_params = jax_opt.adam(10**(-3))
resultsb = train(resultsa[3], 100, 10**3, 500, 10, find_step_size(resultsa[3], 1))

resultsc = train(resultsb[3], 250, 6400, 1000, 10, find_step_size(resultsb[3], 1))

opt_init, opt_update, get_params = jax_opt.adam(10**(-4))
resultsd = train(resultsc[3], 50, 10000, 1000, 100, find_step_size(resultsc[3], 1)) 
...
filename = "g_sigma_exact/g_" + str(g) + ".txt"
