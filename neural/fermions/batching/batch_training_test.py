...
def batch_step(params_arg, step_num, N,N_batches, thermal, skip, variation_size, g):
    ...
    # parallelize this for loop using multiprocessing

    # for i in range(N_batches):
    #     gr = gradient(params_arg, g, samples_per_batch, thermal, skip, variation_size)
    #     grads.append(gr[0])
    #     energies.append(gr[1])
    #     uncerts.append(gr[2])

    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(gradient, params_arg, g, samples_per_batch, thermal, skip, variation_size) for _ in range(N_batches)]
    ...
    grad = jnp.mean(jnp.array(grads), axis=0)
    # map the command gv.gvar(energy, uncert) across the energies and uncertainties
    # to get the gvars
    e_gvars = [gv.gvar(energies[i], uncerts[i]) for i in range(N_batches)]

    # now average the gvars
    energy_gvar = np.mean(np.array(e_gvars))
    # now extract the energy and uncertainty
    energy = energy_gvar.mean
    uncert = energy_gvar.sdev
    ...
    # us.append(gr[2])
    opt_state = opt_init(params_arg)
    new = opt_update(step_num, grad, opt_state)
    return get_params(new), energy, uncert
...
def batch_train(params, iterations, N,N_batches, thermal, skip, variation_size, g):
    ...
    old_params = params.copy()
    for step_num in pbar: 
        new_params, energy, uncert = batch_step(old_params, step_num, N, N_batches, thermal, skip, variation_size, g)
        hs.append(energy)
        us.append(uncert)
        old_params = new_params.copy()
        # save the energies and uncertainties to a file
...
if __name__ == "__main__":
    ...
    step_size = .9
    resultsa = batch_train(params, 10, 100000, 4, 5000, 10, step_size, g)
    ...
    # + resultsb[0] + resultsc[0]  + resultsd[0]  + resultse[0]
    # + resultse[0]

    # + resultsd[0]
    total_uncerts = resultsa[1] 
    # + resultsb[1]  + resultsc[1]  + resultsd[1] + resultse[1]
    # + resultse[1]
